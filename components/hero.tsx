...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    // ensure flags are set on the element before attempting to play
    video.muted = true;
    ...
    const tryPlay = async () => {
      try {
        ...
        // wait a bit before showing fallback — some browsers delay autoplay decisions
  ...
  return (
    <section className="relative w-full h-[700px] md:h-[800px] min-h-screen flex items-center justify-center overflow-hidden">
      <video
        ...
        className={`object-cover w-full h-full absolute inset-0 ${showFallback ? "hidden" : ""}`}
      ...
      >
        <source src="https://bantutech.online/api/chui.mp4" type="video/mp4" />
      </video>
